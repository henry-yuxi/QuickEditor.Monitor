namespace QuickEditor.Monitor
{
    ...
    public enum UnityAssetProcessType
    {
        Preprocess,
        Postprocess
    }

    public class QuickAssetFileInfo
    {
        ...
        public UnityAssetProcessType AssetProcessType { get; private set; }
        ...
        private static Dictionary<UnityAssetType, string[]> assetExtensions = new Dictionary<UnityAssetType, string[]>() {
            ...
            { UnityAssetType.Text, new string[] {".txt", ".xml", ".json" } },
        ...
        public QuickAssetFileInfo(string path)
        {
        ...
        public QuickAssetFileInfo(string path, UnityAssetProcessType assetProcessType)
        {
            this.Name = Path.GetFileName(path);
            this.FullName = path;
            this.DirectoryName = Path.GetDirectoryName(path);
        ...
        public string AssetsRelativePath
        {
            get
            {
                ...
                fullPath = GetFormatPath(fullPath);
                return FileUtil.GetProjectRelativePath(fullPath);
            }
        }

        private string GetFormatPath(string _path)
        {
        ...
        {
            if (obj is QuickAssetFileInfo)
                return Guid == ((QuickAssetFileInfo)obj).Guid;
            else
                return false;
        }

        public static bool operator ==(QuickAssetFileInfo x, QuickAssetFileInfo y)
        {
